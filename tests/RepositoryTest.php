...
{
	/**
	 * @var \Rawilk\LaravelModules\Laravel\Repository
	 */
	private $repository;
	...
	/** @test */
	public function it_adds_locations_to_paths()
	{
	...
	/** @test */
	public function it_find_a_module_by_alias()
	{
		$this->repository->addLocation(__DIR__ . '/stubs/valid');
	...
	public function it_gets_the_configured_stubs_path_if_enabled()
	{
		...
		$this->assertEquals(str_replace('/', '\\', base_path('vendor/rawilk/laravel-modules/src/Commands/stubs')), str_replace('/', '\\', $this->repository->getStubPath()));
	}
	...
	public function it_can_enable_a_module()
	{
		...
		// Disable module to be re-enabled
		$this->repository->disable('Recipe');
	...
	public function it_can_delete_a_module()
	{
		...
		$this->assertTrue(is_dir(base_path('Modules/Blog')));
